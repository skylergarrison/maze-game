...
public class MazeFileReader {
	 ...
	 */
	public MazeFileReader(String filename, Context context) {
	 ...
	 */
	private void load(String filename, Context context)
	{
		try{

			File fXmlFile = new File(context.getFilesDir(), filename);
			Log.v("Fileread@", "Filepath: "+fXmlFile.getAbsolutePath());
			...
			Log.v("Length number", "Number: " + nList.getLength());
	...
	private BSPNode readBSPNode(Element eElement) {
		...
		Log.v("Load number", "Number: " + number);
		if(number<99){
			Log.v("Loading", "in message");
			Message msg = Message.obtain();
			Bundle b = new Bundle();
			b.putInt("prog", number); 
	        msg.setData(b);
		...
		/*Log.v("Loading", "in bsp");
		if((length/100)%number==0){
			Log.v("Loading", "in message");
			Message msg = Message.obtain();
			Bundle b = new Bundle();
			b.putInt("prog", number); 
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		result.col = GraphicsWrapper.getColorValues(col);
	...
	private static void compareSegments(ArrayList<Seg> slist, ArrayList<Seg> slist2) {
		...
		for (int i = 0 ; i < n ; i++)
		{
			...
			if(GraphicsWrapper.getStatRGB(s.col) != GraphicsWrapper.getStatRGB(s2.col)) 
