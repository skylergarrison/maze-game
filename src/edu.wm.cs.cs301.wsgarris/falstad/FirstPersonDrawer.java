...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 * @param mazew 
	 * @param mazeh 
	 * @param gc    MEMO: unlikely to be known when constructor is called, likely to by null
	 */
	 ...
	 */
	@Override
	public void redraw(GraphicsWrapper mp, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		// if notified by model that state has changed
		// Query model for parameters
		//dbg("viewer.redraw called");
		
		//gc = mp.getBufferGraphics();
		gwrap = mp;
		...
		// update graphics
				...
				gwrap.setColorString("black");
				gwrap.wrapFillRect(0, 0, view_width, view_height/2);
				// draw dark gray background on upper half
				gwrap.setColorString("darkgray");
				gwrap.wrapFillRect(0, view_height/2, view_width, view_height/2);
				// set color to white and draw what ever can be seen from the current position
				//gc.setColor(Color.white);
				gwrap.setColorString("white");
				//
		//gwrap.wrapperFPRedraw(mp, state, px, py, view_dx, view_dy, walk_step, view_offset, rset, ang);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		while (x1i <= x2) {
			...
			gwrap.wrapFillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
