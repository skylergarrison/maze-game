...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(GraphicsWrapper mp, int state, int px, int py,
	...
	public void draw_map(int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if(seencells.hasWallOnTop(x, y)){
					gwrap.setColorString("white");
				}else{
					gwrap.setColorString("gray");
				}
				
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					//gc.drawLine(nx1, ny1, nx2, ny1);
					gwrap.wrapDrawLine(nx1, ny1, nx2, ny1);
				...
				if(seencells.hasWallOnLeft(x, y)){
					gwrap.setColorString("white");
				}else{
					gwrap.setColorString("gray");
				}
				...
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					...
					gwrap.wrapDrawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
	...
	public void draw_currentlocation(int view_dx, int view_dy) {
		...
		//draw the location. the center position is adjusted to the center of the view.
		gwrap.wrapFillOval(ctrx-(cirsiz/2), ctry-(cirsiz/2)-1, cirsiz, cirsiz);//(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		//changed arrow to be drawn with a filled polygon because it looks better on screen.
		
		int[] sendA = new int[]{aptx2-ptofx, aptx, aptx2+ptofx};
		int[] sendB = new int[]{apty2-ptofy, apty, apty2+ptofy};
		
		gwrap.wrapFillPolygon(sendA, sendB, 3);
		
		//old line-drawn arrow
		/*//gc.drawLine(ctrx, ctry, aptx, apty);
		gwrap.wrapDrawLine(ctrx, ctry, aptx, apty);
		//gc.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		gwrap.wrapDrawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		//gc.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
		gwrap.wrapDrawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);*/
	}
	...
	public void draw_solution(int offx, int offy, int px, int py) {
		...
		while (d > 1) {
			...
			gwrap.wrapDrawLine(nx1, ny1, nx1+ndx, ny1+ndy);
